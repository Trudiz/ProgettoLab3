...
public class ClientMain2 {
    ...
    public static void main(String[] args) {
        try (Socket s = new Socket("localhost", 9000);
            ...
            Thread listenerThread = new Thread(new UDPListener(udpSocket));
            ...
            while ((msg = console.readLine()) != null && !msg.equalsIgnoreCase("exit")) {
                ...
                RequestMessage requestMsg = createRequest(msgParts, localUdpPort);
    ...
    static public RequestMessage createRequest(String[] tokens, int localUdpPort) {
        ...
        switch(tokens[0]) {
            case "register":
                if (tokens.length == 3 && !clientIsRegistered) {
                    ...
                    RegisterPayload p = new RegisterPayload(tokens[1], tokens[2], localUdpPort);
            ...
            case "login":
                if (tokens.length == 3 && !clientIsRegistered) {
                    //DA RIMUOVERE DOPO
                    clientIsRegistered = true;
                    LoginPayload p = new LoginPayload(tokens[1], tokens[2], localUdpPort);
                    request.setOperation(tokens[0]);
                    request.setPayload(p);
                } else  {
                    System.out.println("Wrong Number of arguments!");
            ...
            case "getactiveorders":
                if (tokens.length == 1 && clientIsRegistered) {
                    GetActiveOrdersPayload p = new GetActiveOrdersPayload();
                    request.setOperation(tokens[0]);
                    request.setPayload(p);
                    break;
                }  else {
                    System.out.println("Wrong Number of arguments for getactiveorders!");
                }
                break;
            case "logout":
                if (tokens.length == 1 && clientIsRegistered) {
